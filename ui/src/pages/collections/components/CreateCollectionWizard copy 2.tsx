...
export default function CreateCollectionWizard({
  ...
  const handleNext = async () => {
    if (current === 0) {
      ...
      // extra validation: upload must have at least one file
  ...
  // Build a config for non-local/browser remote sources (for download)
  const buildConfig = () => {
    ...
    if (source.source_type === "local") {
      ...
      cfg.source.files = fileList.map((f) => f.webkitRelativePath || f.name);
    }
    return cfg;
  };
  ...
  const createCollection = async () => {
    ...
    if (source.source_type === "local") {
      ...
      // Browser upload → multipart to /api/ingest/upload
      const ingest = ingestForm.getFieldsValue();
      const fd = new FormData();
      fd.append("collection", schema.name);
      fd.append("model", ingest.model);
      ...
      // Attach files (preserve relative path if folder selected)
      fileList.forEach((f) => {
        const file = f.originFileObj as File;
        if (file) {
          // Optional: pass relative path as a second field
      ...
      try {
        await mutateAsync({
          ...
          method: "post",
          values: fd,
          meta: { headers: { /* axios will set boundary automatically */ } },
          successNotification: () => ({ message: "Upload started", description: "Files are being ingested." }),
          errorNotification: (err) => ({
            message: "Upload failed",
            description: (err as any)?.response?.data?.detail || String(err),
            type: "error",
          }),
          // refine doesn't expose progress directly; simulate linear progress for UX
        });
        setUploadPct(100);
      } finally {
        setUploading(false);
                  ...
                  <>
                    <Space direction="vertical" style={{ width: "100%" }}>
                      ...
                      <Upload.Dragger
                        ...
                        fileList={fileList}
                        beforeUpload={(file) => {
                          // Prevent auto upload; store locally
        ...
        <Result
          ...
          subTitle={
            <>
              ...
              {sourceType === "upload" ? (
                <div>Files were uploaded for ingestion.</div>
              ) : syncTriggered ? (
